# Features:
#   • Natal Chart + Ascendant (Whole Sign Houses)
#   • Current Planetary Positions
#   • Current Panchang (Tithi, Yoga, Karana, Nakshatra, Sunrise/Sunset)
#   • Mahadasha + Full Antardasha list + Full Pratyantardasha (current + all lists)
#   • 60+ Important Yogas detected with description
#   • Sunrise/Sunset for birth day & current day
#   • Lahiri Ayanamsa, Swiss Ephemeris precision
...
app = FastAPI(title="AstroVed God-Level Vedic API")
...
def detect_yogas(planets: dict, lagna_lon: float):
    ...
    mahapurusha = {
        "Ruchaka": ("Mars", [0, 300]),
        "Bhadra": ("Mercury", [150, 170]),
        "Hamsa": ("Jupiter", [60, 240]),
        "Malavya": ("Venus", [30, 210]),
        "Sasa": ("Saturn", [300, 330])
    }
    for name, (planet, signs) in mahapurusha.items():
        if any(abs(planets[planet] - s) < 30 for s in signs) and h[planet] in [1,4,7,10]:
            yogas.append({"yogaName": f"{name} Yoga", "description": f"{planet} in own/exaltation in kendra", "present": True})

    # Gaja Kesari
    jm_diff = min(abs(planets["Jupiter"] - planets["Moon"]), 360 - abs(planets["Jupiter"] - planets["Moon"]))
    if 80 < jm_diff < 100 or 260 < jm_diff < 280:
        yogas.append({"yogaName": "Gaja Kesari Yoga", "description": "Jupiter & Moon in mutual kendras - fame & wealth", "present": True})

    # Budhaditya
    if abs(planets["Sun"] - planets["Mercury"]) < 13:
        yogas.append({"yogaName": "Budhaditya Yoga", "description": "Sun-Mercury conjunction - brilliant intellect", "present": True})

    # Chandra Mangala
    if abs(planets["Moon"] - planets["Mars"]) < 12:
        yogas.append({"yogaName": "Chandra Mangala Yoga", "description": "Wealth through courage", "present": True})

    # Vipareeta Raja Yoga
    if any(h[p] in [6,8,12] for p in ["6thLord", "8thLord", "12thLord"] if p in h):
        yogas.append({"yogaName": "Vipareeta Raja Yoga", "description": "Sudden rise after struggle", "present": True})

    # Kaal Sarpa
    rahu, ketu = planets["Rahu"], planets["Ketu"]
    all_between = all(min(abs(planets[p] - rahu) % 360, abs(planets[p] - ketu) % 360) < 180 for p in ["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn"])
    if all_between:
        yogas.append({"yogaName": "Kaal Sarpa Yoga", "description": "All planets between Rahu-Ketu axis", "present": True})

    # Dhana Yoga
    if h["2ndLord"] in [1,2,5,9,11] or h["11thLord"] in [1,2,5,9,11]:
        yogas.append({"yogaName": "Dhana Yoga", "description": "Great wealth accumulation", "present": True})

    # Raja Yoga (kendra-trikona link)
    if any(h[a] == h[b] for a in [1,4,7,10] for b in [1,5,9]):
        yogas.append({"yogaName": "Raja Yoga", "description": "Power, status, success", "present": True})
...
def get_dasha_details(moon_lon: float, jd_birth: float, tz_str: str):
    ...
    now_jd = swe.julday(datetime.utcnow().year, datetime.utcnow().month, datetime.utcnow().day, datetime.utcnow().hour + datetime.utcnow().minute/60 + datetime.utcnow().second/3600)
    ...
    while True:
        ...
        if m_end_jd >= now_jd:
            m_start = jd_to_datetime(jd, tz_str)
            m_end = jd_to_datetime(m_end_jd, tz_str)
            ...
            for a in range(9):
                ...
                a_start = jd_to_datetime(jd_a, tz_str)
                a_end_jd = jd_a + a_years * 365.242189
                a_end = jd_to_datetime(a_end_jd, tz_str)
                is_current = jd_a <= now_jd < a_end_jd
                antar_list.append({
                    "antardasha": a_lord + (" (Current)" if is_current else ""),
                    "startDate": a_start.strftime("%Y-%m-%d %I:%M %p"),
                    "endDate": a_end.strftime("%Y-%m-%d %I:%M %p")
                })

                if is_current:
                    current_antar = a_lord
                    current_a_start = a_start
                    current_a_end = a_end

                    # Pratyantardasha
                    jd_p = jd_a
                    elapsed_p = now_jd - jd_a
                    for p in range(9):
                        p_idx = (cl + a + p) % 9
                        p_lord = LORDS[p_idx]
                        p_years = a_years * YEARS[p_idx] / 120
                        p_start = jd_to_datetime(jd_p, tz_str)
                        p_end_jd = jd_p + p_years * 365.242189
                        p_end = jd_to_datetime(p_end_jd, tz_str)
                        is_curr_p = jd_p <= now_jd < p_end_jd
                        praty_list.append({
                            "pratyantardasha": p_lord + (" (Current)" if is_curr_p else ""),
                            "startDate": p_start.strftime("%Y-%m-%d %I:%M %p"),
                            "endDate": p_end.strftime("%Y-%m-%d %I:%M %p")
                        })
                        jd_p = p_end_jd

                jd_a = a_end_jd

            current_praty = next((i["pratyantardasha"].split(" ")[0] for i in praty_list if "(Current)" in i["pratyantardasha"]), "Unknown")

            return {
                "mahadasha": m_lord,
                "mahadashaStart": m_start.strftime("%Y-%m-%d %I:%M %p"),
                "mahadashaEnd": m_end.strftime("%Y-%m-%d %I:%M %p"),
                "currentAntardasha": current_antar,
                "currentAntardashaStart": current_a_start.strftime("%Y-%m-%d %I:%M %p") if current_a_start else "",
                "currentAntardashaEnd": current_a_end.strftime("%Y-%m-%d %I:%M %p") if current_a_end else "",
                "antardashaList": antar_list,
                "currentPratyantardasha": current_praty,
                "currentPratyantardashaStart": [i["startDate"] for i in praty_list if "(Current)" in i["pratyantardasha"]][0] if any("(Current)" in i["pratyantardasha"] for i in praty_list) else "",
                "currentPratyantardashaEnd": [i["endDate"] for i in praty_list if "(Current)" in i["pratyantardasha"]][0] if any("(Current)" in i["pratyantardasha"] for i in praty_list) else "",
...
def full_vedic_chart(data: BirthInput):
    ...
    now_jd = swe.julday(datetime.utcnow().year, datetime.utcnow().month, datetime.utcnow().day, datetime.utcnow().hour + datetime.utcnow().minute/60 + datetime.utcnow().second/3600)
    current = {name: (swe.calc_ut(now_jd, pid)[0][0] - swe.get_ayanamsa_ut(now_jd)) % 360 for name, pid in pids.items()}
    ...
    current_sunrise, current_sunset = get_sunrise_sunset(now_jd, data.latitude, data.longitude, data.timezone)
    ...
    return {
        ...
        "currentPanchang": {
            ...
            "currentTithi": {"currentTithiName": get_tithi(now_jd)},
            "currentKarana": {"currentKaranaName": get_karana_name(now_jd)},
            "currentYoga": {"currentYogaName": get_yoga_name(now_jd)},
...
@app.get("/")
def home():
    return {"status": "AstroVed Ultimate Vedic API - Complete with Yogas, Pratyantardasha, Panchang - Nov 21 2025"}
